...
public class JeuFrame extends JFrame {
	...
	private void attribuerJoueur() {
		...
		nom = (String) JOptionPane.showInputDialog(null, "Nom du joueur 1 ?", "Fiche d'inscription", JOptionPane.INFORMATION_MESSAGE, imgJ1, null, null);
		if(nom == null) {
			nom = nom1;
		}
		...
		nom = (String) JOptionPane.showInputDialog(null, "Nom du joueur 2 ?", "Fiche d'inscription", JOptionPane.INFORMATION_MESSAGE, imgJ2, null, null);
		if(nom == null) {
			nom = nom2;
		}
		...
		nom = (String) JOptionPane.showInputDialog(null, "Nom du joueur 3 ?", "Fiche d'inscription", JOptionPane.INFORMATION_MESSAGE, imgJ3, null, null);
		if(nom == null) {
			nom = nom3;
		}
			...
			if(!j.isDeplacerBinome()) {
				if(!containerJeu.isEnabled()) {
					if(!j.isCarteJoue()) {
						...
						}else if(j.getJoueurActif().getNbUV() <= 0){
							infosPartie.setText("" + j.getJoueurActif().getNom() + " ne possede plus assez d'uv !");
						}else{
							infosPartie.setText("Phase de fondation : \n Vous ne pouvez pas contruire ici !");
						}
					}else{
						plat.fonder(e.getPoint(), j);
						j.setNbUvPose(j.getNbUvPose()-1);
						if(j.getNbUvPose() <= 0) {
							...
							infosPartie.setText("Vous ne pouvez pas contruire ici !");
						}
					}
				}else{
					if(plat.ajouter(e.getPoint(), getJ())){
						infosPartie.setText("C'est à " + j.getJoueurActif().getNom() + " de jouer ! \n Nouvelle construction !");
					}else if(j.getJoueurActif().getNbCC() <= 0 || j.getJoueurActif().getNbUV() <= 0 || j.getJoueurActif().getNbUVStar() <= 0){
						infosPartie.setText("" + j.getJoueurActif().getNom() + " ne possede plus assez de pions !");
					}else{
						infosPartie.setText("C'est à " + j.getJoueurActif().getNom() + " de jouer ! \n Vous ne pouvez pas contruire ici !");
					}
				}
				...
				if(plat.attribuerBinome(e.getPoint())){
					infosPartie.setText("Le binome glandeur à été déplac par " + j.getJoueurActif().getNom() + " !");
					if(j.isCarteJoue()) {
						j.volerCartes(false);
						j.setCarteJoue(false);
					...
					infosPartie.setText("Le binome glandeur ne peut pas être place ici !");
				}
			}
			updateJoueurs();
			updateReserve();
	...
	private class ActionButtonDes implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(j.getValeurDes() != 7) {
				if (j.attribuerRess()) {
					infosPartie.setText("Les ressources ont ete attribuees chaques participants !");
				}else{
					infosPartie.setText("Les ressources n'ont pas ete attribuees a chaque participants !");
				}
			}else{
	...
	private class ActionPlayCarte implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			SousTypeCartes sstype = (SousTypeCartes) JOptionPane.showInputDialog(null, 
													  "Quel cartes de developpement souhaitez vous jouer ?",
													  "Jouer une carte de developpement",
													  JOptionPane.QUESTION_MESSAGE,
			...
			if(sstype == null){
				infosPartie.setText("" + getJ().getJoueurActif().getNom() + " n'a pas pu jouer de cartes de developpement !");
			}else if(sstype == SousTypeCartes.ANCIEN){
				infosPartie.setText("" + getJ().getJoueurActif().getNom() + " a jouer une carte de developpement :  " + sstype + " ! \n Le binome glandeur doit être déplacé sur un nouvel Hexagone!!!");
				j.setCarteJoue(true);
				j.setDeplacerBinome(true);
				enabledButton(containerJeu, false);
	...
	private class ActionExchange implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(j.echangerCartes(edDial.getRess1(), edDial.getNb1(), edDial.getRess2(), edDial.getNb2(), getJ().getJoueur(edDial.getNom()))){
				...
				infosPartie.setText("" + getJ().getJoueurActif().getNom() + " n'a pas echange avec " + edDial.getNom() + " ! \n Problème de ressources !");
			}
			updateJoueurs();
		}
	}
