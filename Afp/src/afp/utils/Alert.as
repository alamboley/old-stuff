package afp.utils
{
	...
	public class Alert
	{
		...
		private static var btnHeight : int = 50;
		...
		private static function createBackground(alertOptions : AlertOptions) : Sprite
		{
			...
			switch (alertOptions.background)
			{
				case "blur" :
					...
					break;
				case "none" :
					myBackground.graphics.beginFill(colour, 0);
					// BACKGROUND IS STILL THERE BUT IS INVISIBLE
		...
		private static function createPrompt(alertOptions : AlertOptions) : Sprite
		{
			...
			promptBackground.graphics.drawRoundRect(0, 0, myWidth, myHeight + 100, ellipseSize, ellipseSize);
			...
			{
				alertButtons[i].y = actualPrompt.height - btnHeight - 20;
